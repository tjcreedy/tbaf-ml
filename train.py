...
from sklearn import (feature_selection, decomposition, model_selection,
                     svm, metrics, pipeline)
...
def analyse_results(search, scorers, output, train, cls, strat):
    ...
    for rs in scorers.keys():
        ...
        i = [i for i, p in enumerate(search.cv_results_['params']) if p == bp][0]
            #Curves
        for c in curves:
            # c = 'prc_values'
        ...
        ax.set_title( "Mean and Standard Deviation Confusion Matrix "
                     f"values for the {search.n_splits_}\ntest-train split "
                     f"model fittings with the best {rs}")
        pdf.savefig(fig)
        
        # Save final model
        models[rs] = search.best_estimator_
    ...
    ax.set_title('Receiver Operating Characteristic Curves for models best\n'
                 'optimised for different scores')
    ...
    ax.set_title('Precision-Recall Curves for models best optimised\nfor '
                 'different scores')
    ax.legend(loc = 'lower right')
    pdf.savefig(fig)
    ...
    # TODO: plot mean value of scores for values of C and tol 
...
def plot_kfold_curves(dicts, n, score):
    ...
    title = (f" curves for the {n} split model\nfittings with the "
             f"best {score}")
...
def main():
    ...
    pipe = pipeline.Pipeline([('reduce_dim', 'passthrough'),
                              ('classify', svm.LinearSVC(dual = False, 
...
#            ]
    ...
    param_grid = [
            dict({f"classify__{k}":v for k,v in linsvc_params.items()}, **p)
                for p in param_grid
                ]
    ...
    models = analyse_results(gs, scorers, args.output, train, cls,
                             strat)
