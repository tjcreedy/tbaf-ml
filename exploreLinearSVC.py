...
def analyse_results(search, scorers, output, new, train, cls, data_train, 
    ...
    # Set up outputs
    pred = pd.DataFrame(index = new.index)
    retain = dict()
    ...
    for rs in scorers.keys():
        ...
        for c in ['roc_values', 'prc_values', 'threshc_values']:
            # c = 'roc_values'
            pdf.savefig(plot_kfold_curves(search.cv_results_[c][i], 
                                          search.n_splits_,
                                          rs))
        
        # Use this fit to predict for new
        pred[f"prediction_{rs}"] = search.predict(new)
        pred[f"decision_{rs}"] = search.decision_function(new)
        retain[rs] = pred.index[pred[f"prediction_{rs}"] == 1].tolist()
...
def plot_kfold_curves(dicts, n, score):
    ...
    fig, ax = plt.subplots()
    sumvals = []
    for d, c in zip(dicts, colours):
        # d, c = list(zip(dicts, colours))[0]
        ax.plot(d[x], d[y1], c = c)
        if y2:
            ax.plot(d[x], d[y2], '--', c = c)
        if v:
            sumvals.append(d[v])
    if y2:
        ax.set_xscale('symlog')
...
def main():
    ...
    retentions = analyse_results(grid_search, scorers, args.output, new, 
                                 train, cls, data_train, cls_train, data_test,
                                 cls_test)
